  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(socket: SocketWithAuth) {
    ...
    this.logger.debug(
      `Total participant connected to room '${roomName}': ${connectedClients}`,
    );
  ...
  async handleDisconnect(socket: SocketWithAuth) {
    ...
    this.logger.debug(
      `Total participant connected to room '${socket.pollID}': ${clientCount}`,
    );
  ...
  async addOption(
    ...
    this.logger.debug(
      `Adding option by user ${client.userID} to poll ${client.pollID}\n${option.text}`,
    );
  ...
  async removeOption(
    ...
    this.logger.debug(
      `Attempting to remove option ${optionID} from poll ${client.pollID}`,
    );
